...
public class StudentService {
	...
	public void displayMenu() {
		int input = 0; //scanner로 입력 받은 메누 번호를 저장할 변수
...
//				System.out.println("정보를 조회할 변수를 선택 (1:std1 / 2:std2)  : ");
		...
		} while(input != 0); // input이 0이 아닐 떄 반복
	}
	...
	/** 매개 변수로 잔달 받은 학생의 정보를 학생 문자열로 만들어서 반환
	...
	private String studentInform(Student s) {
		...
		// - 패턴 모양의 문자열을 반환하는 String 메서드
																									//java / html
		//KH대학교 / 123123 / 남 / 홍길동 / java / 0 / 0
		return String.format("%s /%s / %c / %s / %d / %d" , 
				Student.schoolName, s.getStudentNumber(), s.getStudentgender(), s.getStudentName(),s.getJava(), s.getHtml()  );
	}
	...
	private void updateStudentName(Student s) {
		...
		String beforeName = s.getStudentName();
		s.setStudentName(newName); // 새이름을 세팅
	 ...
	 * 학생의 Java 역량을 수정
	 * 단ㄴ, 수정된 Java 역량은 최대값, 최소값 범위를 넘어가지 못하게함
	 * 

...
public class PolyService {
	 ...
	 * 
	 * 2. 오버 로딩:
	 * 	- > 같은 클래스 에서 같은 이름의 메서드를 여러 번 작성하는 기술
...
//		//자식 기능 호출 -> 가능 O / X
//		s1.setIosVersion(17);
//		s1.setAndroidVersion(15);
		
		//The method setIosVersion(int) is undefined for 
		// the type SmartPhone
		// -> SmartPhone 에 setIosVersion()이 정의되지 않음( == 없다)
	}
...
//		int v2 = s2.getAndroidVersion();
		...
		//다운 캐스팅 + 얕은 복사(주소만 복사)
		Iphone i1 = (Iphone)s1; // s1을 Iphone 으로 강제 형변환 후 
																			// 주소를 i1 대입
	 ...
	 *   호출 되는 메서드가
	 *   어떤 클래스의 메서드와 연경되는지 결정하는 것
	 * 	 */
	public void method3() {
		SmartPhone s1 = new SmartPhone();
		
		// SmartPhone의 setApt() 메서드와 연결된다
		//void.edu.kh.poly.pack1.model.d
		s1.setAp("M1");
		
		Galaxy g1 = new Galaxy();
		
		
		//Galxy 클래스의 setAndroidVersion() 메서듸와 연결
	 ...
	 *  	객체의 오버라이딩 된 메서드로 연결
	 *  
	 *  (부모 타입 참조 변수 = 자식 객체 (업캐스텡)
	 *  
	 *  -> 부모 타입이 참조하는 객체는 실제로는 자식 타입 객체 !!
	 *  
	 *  -> 오버라이딩된 메서드 호출 시 (부모, 자식이 서로 같은 이름의 메서드를 가비고 있음)
	 *  
	...
	/**
	 * 객체 배열 + 다형성(업케스팅) + 다형성 (동적 바인딩)
	 */
	public void method5() {
		//부모 타입 찹조 변수로 이루어진 객체 배열 생성
    ...
    // SmartPhone(부모), Iphone(자식),Galaxy(자식)
		...
		//부모 참조 변수  = 부모 객체 
		arr[0] = new SmartPhone("LCD" , "KT" ,"엑시녹스");
	...
	public void method6() {
		...
		Galaxy galaxy = new Galaxy("AMOLED", "LG", "스냅드래곤", 14);
...
//		System.out.println(s1 instanceof PolyService);
	...
	public void method9() {
		...
		// s가 참조하고 있는 객체가 Galaxy 객체가 맞아?
			...
			else { // Iphone 도 Galaxy도 아닌 경우 == smartPhone
				s.setDisplay("부모 화면");
			}
		}
		
		//향상된 for 문 + 매개 변수 다형성 (업캐스팅) + 동적 바인딩
		for(SmartPhone s : arr) {
			printToString(s);
		}
	}

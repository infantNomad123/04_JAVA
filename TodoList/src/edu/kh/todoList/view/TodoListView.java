...
public class TodoListView {
	...
	public TodoListView() {
		...
		//객체 생성 시 발생되는 예외는 여기서 모아서 처리할 예정
	...
	private void todoAdd() throws IOException {
		...
		while(true) {
			...
			if(content.equals("!wq"))  break; // !wq 입력 반복 종료
...
//--------------------------------------------------------------------------------------
	...
	private void todoComplete() throws NumberFormatException, IOException {
		...
		//서비스 호출하고 경과(T/F) 반환 받기
		boolean result = service.todoComplete(index);
		
		if(result) { // 변경 성공
			System.out.println("[변경 되었습니다]");
			
		}else { // index 요고가 범위를 초과한 경우

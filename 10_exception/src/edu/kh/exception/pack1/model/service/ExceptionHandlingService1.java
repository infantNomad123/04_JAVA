 ...
 * 
 * Exception Handling(예외 처리) : Excepion 발생 시 처리하는 구문
 * 
 */
	...
	public void method1() {
		...
		//java.lang.SrrayIndexOutOfBoundsException:
		// Index 4 out of bounds for length 4
		
		/*예외 발생 원리*/
		// 실행 중 예외 상황이 발생하는 경우
		// 에외 상황에 맞는 코드가 수행되며
		// 지정된 예외 클래스를 찾아 객체로 만들어사 던짐
	...
	public void method3() {
		...
		try {
			...
			//try 에서 던뎌디는 ArithmeticException 객체를 
			// 매개 변수를 이용해 잡음
	...
	public void method4() {
		...
		// -Scanner 를 이용해서 int, long, float, double 등
		// 특정 자료형 형태의 문자를 입력 받으려고 했는데
		// 입력한 값이 다른 자료형일 경우 발생되는 (던져지는) 예외
	System.out.println("[두 수를 입력 받아 나눈 몫 구하기]");
	...
	public void method5() {
		...
		do {
			...
			//  자식 예외를 부모 catch 문에서 모두 잡아서 처리하기 때문에 
			//  자식 catch 문 까지 코드가 도달할 수 없다! (수행 불가, 죽은 코드)
			// -> 해결 방법 : 부모 catch를 자식 catch 밑에 작성
//			catch(RuntimeException e) {
//				System.out.println("-------실행 중 발생한 예외 처리 완료-------");
//			}
		
			//Unreachable catch block : 도달할수 없는 catch 구문
			catch(InputMismatchException e) {
				...
				input = -1 ; // 첫 반복에서 종료되지 않게 input 값을 수정
			}
	...
	/**
	 * catch 문에 부모 타입 예외 처리 구문 작성하기 (다형성)
	 * 
	 * Exception 클래스
	 *  - 모든 예외의 최상위 부모 클래스
	 *  
	 *  - catch 문에 Exception으로 예외를 받기로 하면

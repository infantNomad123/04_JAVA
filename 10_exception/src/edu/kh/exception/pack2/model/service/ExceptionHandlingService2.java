...
public class ExceptionHandlingService2 {
	/*finally : 마지막애 반드시 수행하는 구문
	 * 
	 * - try구문에서 예외 발생 여부와 관계 없이 
	 * 		무조건 수행해야죄는 구문을 작성
	 * 
	 ...
	 */
	public void method1() {
		System.out.print("1. 정상수행 / 2. 강제 예외 발생");
		
		int input = sc.nextInt();
		
		try {
			if(input == 1) System.out.println("[정상 수행]");
			else              throw new RuntimeException();//예외 객체 생성 후 던짐(throw)
		}		catch(Exception e){ 
	...
	public void method2() {
		...
		//[해결 방법] Scanner 를 close (메모리 반환) 하기
		// -> 코드에서 예외가 발생 하든 말든 무조건 close 수행 필수!!!

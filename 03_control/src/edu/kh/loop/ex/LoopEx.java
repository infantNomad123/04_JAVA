...
public class LoopEx {
		 ...
		 * 
		 * 조건1 : B가 A보다 큰 경우  -> A ~ B 까지 1 씩 증가하며 출력
		 * 
		 * 조건2 : A가 B보다 큰 경우  -> A ~ B 까지 1 씩 감소하며 출력
		 * 
...
//			Scanner sc = new Scanner(System.in);
		 ...
		 * 두 수를 입력받아 반복 출력 하기
		 * 
		 * (첫 번째 입력 A , 두 번 째 입력 B)
		 * 
		 * -입력 받은 두 수 중
		 * 	작음 값이 초기식, 큰 값이 조선식으로 대입되게 작성
		 * 	(같은 경우는 초기식 A, 조건식 B)
		 */
		public void method5() { 
			System.out.println("입력 (1 2) :");
	...
	public void method6() {
		int count = 0;
		for(int i = 1; i<=100; i++) {
			if(i % 7 == 0) { //i 가 7의 배수안 경우 count 1 증가
				count ++;
			}
		}

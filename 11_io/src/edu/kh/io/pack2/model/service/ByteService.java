...
//			 new FileOutputStream("경로"):
			...
			//StringBuilder f-> String 변환
			String content = sb.toString();
			
			//출력 방법 1 : 한 글자 (2 byte) 씩 파일로 출력
			// -> 2byte 	범주의 문자 (영어, 숫자, 기본 특수문자 제외)
			//   1byte 기반 스트림 통과 시 데이터 손실이 발생해서
			// 글자가 깨지는 문제가 발생!
//			for(int i =0, len = content.length(); i<len ; i++) {
//				char ch = content.charAt(i);  // i 번쨰 글자 반환
//				fos.write(ch); // 1byte 출력 스트림이 변경된 파일에 ch 쓰기
//			}
	...
	public void bufferedFileByteOutput() {
		...
		try {
			...
			try {
				...
				//보조 스트림 close() 시 
				// 보조 스트림 생성에 상요된 기반 스트림도 같이 close() 됨
	...
	public void fileByteInput() {
		...
		try {
			...
			while(true) { // 무한 반복
				...
				sb.append((char)value); //char 로 강제 형변환해서 글자 형태로 추가
			}
	...
	public void fileByteInput2() {
		...
		try {
			...
			//            byte [] 형태로 반환 받기
			//            -> 이 후 byte 배열을 이용해서 String 갹체 생성
			//            -> String 생성 시 배열 요소 2 개씩 몪어 한글자로 해석해
			//            깨지는 글자 없이 문자열로 변환
	...
	public void bufferedFileByteInput() {
		...
		try {
			...
			//파일 내용을 byte[] 배열로 반환 받아 String 으로 변경
			String content = new String (bis.readAllBytes());
			System.out.println(content);
			
		}catch(Exception e) { //Exception 은 IOException 의 부모 (다형성)
			e.printStackTrace();
	 ...
	 * 
	 * [실행화면]
	 * 파일 경로 입력 : /io_test/202402215/노래가사.txt
	 * 복사 완료 :        /io_test/202402215/노래가사_copy.txt
	 * 
	...
	public void filecopy() {
		...
		BufferedInputStream bis = null; // 파일 보조
		
		FileOutputStream fos = null;     // 파일 출력
		BufferedOutputStream bos = null; // 파일 보조

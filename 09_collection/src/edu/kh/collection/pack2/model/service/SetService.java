...
public class SetService {
	 ...
	 *     (인덱스 업음! -> get() 메서드가 없음)
	 *     
	 *  - 중복 데이터 저장 X (같은 객체가 들어오먄 덮어쓰기)
	 *  
	 *  Set 인터페이스의 자식 클래스
	 *  
	 *  1. HashSet (대표) : 처리 속도가 빠른 Set
	 *  
	 *  2. LinkedHashSet : 순서를 유자헐는 Set
	 *  
...
//		Set<String> set = new LinkedHashSet<String>(); // 다형성 - 업캐스팅
	 ...
	 // 전달 벋은 e를 Set에서 제거 - 제거 성공 true, 없으면 false
...
//		
//		
//		for(int j =1,  result = input/1000; j<= result; j++){
...
//		for문 반복될 때 마다 새로운 Set 객체를 생성
	...
	//출력용 반복문
		for(int j =0 , size = lottoList.size(); j < size; j++) {
			System.out.println((j+1) + "회 : " + lottoList.get(j));
		}
	}
}

...
// 컬랙션 프레임워크: Java에서 자료구조를 만들어서 모아둔 것
//(java.util에 있음)

//특징 
//1) 크기 제한 없음 (부족하면 자동으로 증가)
//2) 추가, 수정, 석제, 검색 등 다양한 가능이 구현되어 있음
//3) 객체만 저장 가능
//       (기본 자료형은 안됨....하지만 Wrapper 클래스를 이용하면된다!)

public class ListService {
	//List : 자료를 순차적으로 나열한 자효 구조 (배열 비슷)
	// - 인덱스가 존재함
	// -> 인덱스 순서가 구분되기 때문에 중복 체이터 저장 가능
	...
	public void method1() {
		// List 객체 생성
		// - List (인터페이스) -> 객체 생성 불가능 -> 다형성 이용
		// - ArrayList (클래스) (List 의 후손)
//		  List list = new ArrayList(); // 10칸 짜리 생성
		  List list = new ArrayList(3); // 10칸 짜리 생성
...
//		  										자료형 가리지 않고 들어갈수 있다
		  ...
		  list.add("아무거나");
		  list.add(new Object());
		  list.add(123123);  // -> int 가 Integer(Wrapper Class) 로 변환
		  													// -> 컴파일러가 자동으로 변환
		  
		  list.add(3.123123);						//크기 3 초과 했을때 오류 발생 확인
		  list.add(false);
		  System.out.println(list);		//-> 오류 x
		  																			// 왜? 크기가 자동으로 늘어나서
		  ...
		  for(int i = 0; i< list.size(); i++) {
		  	...
		  	//List의 i 번째 요소가 String 인 경우
		  	if(list.get(i) instanceof String) {
		  		System.out.println("[String 자료형 입니다]\n");
		  		continue;
		  	}
		  	
		  	//List의 i 번째 요소가 int( -> Integer 변환) 인 경우
		  	if(list.get(i) instanceof Integer) {
		  		System.out.println("***Integer 자료형 입니다*****\n");
		  		continue;
		  	}
		  //List의 i 번째 요소가 double( -> Double 변환) 인 경우
	...
	public void method2() {
		...
		//컬렉션의 장점 중 "여라 타입 저장 가능" 이
		//코드 길이르 늘리는 주범!!!
		// -> 타입 을 하나로 제한 (제네릭) -> 코드 길이 감소
		
		//String 타입으로 재한된 ArrayList 객체 생성
		...
		strList.add("치킨");
		strList.add("보쌈");
//		strList.add(123);//String이 아니라서 에러 발생
		strList.add("족발");
		strList.add("카레돈까스");
		strList.add("한우불고기버거");
		...
		String before = strList.set(2,  "양념치킨");
		 ...
		 System.out.println(strList.contains("보쌈")); //3
		 System.out.println(strList.contains("막국수"));//-1
